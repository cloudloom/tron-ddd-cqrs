...
import com.tracebucket.tron.cqrs.annotation.ApplicationService;
import com.tracebucket.tron.cqrs.annotation.CommandHandler;
import com.tracebucket.tron.cqrs.annotation.ReplyTo;
import com.tracebucket.tron.ddd.annotation.DomainEventHandler;
...
public class ApplicationServiceAutoConfiguration implements ApplicationListener<ContextRefreshedEvent>,
	...
	public void wireBean(final Object bean, final Set<Method> methods) {
		...
		Consumer consumer;
		Bus reactor;
		CommandHandler commandHandlerAnno;
		ReplyTo replyToAnno;
		reactor.bus.selector.Selector selector;

		for (final Method method : methods) {
			//scanAnnotation method
			commandHandlerAnno = AnnotationUtils.findAnnotation(method, CommandHandler.class);
			replyToAnno = AnnotationUtils.findAnnotation(method, ReplyTo.class);
			reactor = fetchBus(commandHandlerAnno, bean);
			selector = fetchSelector(commandHandlerAnno, bean, method);
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Attaching ApplicationService to Reactor[" + reactor + "] using CommandHandler[" + selector + "]");
			}
	...
	private Bus fetchBus(CommandHandler commandHandlerAnno, Object bean) {
		return expression(commandHandlerAnno.eventBus(), bean);
	}

	private Object parseSelector(CommandHandler commandHandler, Object bean, Method method) {
		if (!StringUtils.hasText(commandHandler.value())) {
			return method.getName();
		}

		try {
			return expression(commandHandler.value(), bean);
		} catch (Exception e) {
	...
	private reactor.bus.selector.Selector fetchSelector(CommandHandler commandHandlerAnno, Object bean, Method method) {
		Object sel = parseSelector(commandHandlerAnno, bean, method);
		try {
			switch (commandHandlerAnno.type()) {
				case OBJECT:
					return object(sel);
				case REGEX:
